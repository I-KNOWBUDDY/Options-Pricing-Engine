...
TEST_F(BinomialTreeTest, Greeks) {
    ...
    const double bs_vega = option->vega();
    const double bs_rho = option->rho();

    option->setPricingEngine(bin_engine);
    const double bin_delta = option->delta();
    const double bin_gamma = option->gamma();
    const double bin_theta = option->theta();
    const double bin_vega = option->vega();
    const double bin_rho = option->rho();
    ...
    EXPECT_NEAR(bin_vega, bs_vega, tolerance);
    EXPECT_NEAR(bin_rho, bs_rho, tolerance);
}
