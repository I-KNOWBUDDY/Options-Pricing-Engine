...
TEST_F(MonteCarloTest, GreeksVsBlackScholes) {
    ...
    double bs_vega = option->vega();
    double bs_rho = option->rho();

    option->setPricingEngine(mc_engine);
    double mc_delta = option->delta();
    double mc_gamma = option->gamma();
    double mc_theta = option->theta();
    double mc_vega = option->vega();
    double mc_rho = option->rho();
    ...
    EXPECT_NEAR(mc_vega, bs_vega, tolerance);
    EXPECT_NEAR(mc_rho, bs_rho, tolerance);
}
