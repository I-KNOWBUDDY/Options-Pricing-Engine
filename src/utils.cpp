...
double forwardPrice(double spot, double rate, double dividend, double time) {
    ...
    auto engine = std::make_shared<BlackScholesPricingEngine>();
    OptionType type = isCall ? OptionType::Call : OptionType::Put;
    EuropeanOption option(type, strike, time, spot, rate, 0.5, dividend);
    option.setPricingEngine(engine);

    // Intrinsic value
    double discount_q = std::exp(-dividend * time);
    double discount_r = std::exp(-rate * time);
    double intrinsic = isCall ? (spot * discount_q - strike * discount_r) : (strike * discount_r - spot * discount_q);
    if (intrinsic < 0.0) {
        intrinsic = 0.0;
    }

    if (marketPrice < intrinsic) {
        throw std::invalid_argument("Market price is below intrinsic value");
    }

    double sigma = 0.5; // Initial guess
    for (int i = 0; i < MAX_ITERATIONS; ++i) {
        option.setVolatility(sigma);
        ...
        double vega_math = option.vega() * 100.0;

        if (std::abs(vega_math) > 1.0e-8) {
            double next_sigma = sigma - diff / vega_math;
            if (next_sigma > low && next_sigma < high) {
                sigma = next_sigma;
                continue;
            }
        }
        ...
        option.setVolatility(low);
        double price_low = option.price();
        option.setVolatility(high);
        double price_high = option.price();

        if (marketPrice < price_low) {
            return low;
        }
        if (marketPrice > price_high) {
            return high;
        }

        double mid = 0.5 * (low + high);
        option.setVolatility(mid);
        double price_mid = option.price();
        if (price_mid < marketPrice) {
            low = mid;
        } else {
            high = mid;
        }
...
double  finiteDifferenceDelta(const Option& option, const double h) {
...
double  finiteDifferenceGamma(const Option& option, const double h) {
    const double spot = option.getSpot();
...
double  finiteDifferenceTheta(const Option& option, const double h) {
    ...
    // Correct daily Theta formula using backward difference:
    return (new_price - original_price) / (h * 365.0);
}

double  finiteDifferenceVega(const Option& option, const double h) {
...
double  finiteDifferenceRho(const Option& option, const double h) {
